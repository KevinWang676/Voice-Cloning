...
urllib.request.urlretrieve("https://isv-data.oss-cn-hangzhou.aliyuncs.com/ics/MaaS/TTS/download_files/nltk_data.zip", "nltk.zip")
with zipfile.ZipFile("nltk.zip", 'r') as zipf:
...
BASE_MODEL_DIR = download_model(BASE_MODEL_ID, "v1.0.4")
print("Base model dir path: {}".format(BASE_MODEL_DIR), flush=True)

AUTO_LABEL_RESOURCE_DIR = download_and_unzip_resousrce(DEFAULT_RESOURCE_MODEL_ID, "v1.0.4")
    ...
    try:
        ...
        if local_flag is True:
            custom_infer_abs = {
                ...
                'F7',
                'am_ckpt':
                osp.join(model_dir, 'tmp_am', 'ckpt'),
                'am_config':
                osp.join(model_dir, 'tmp_am', 'config.yaml'),
                'voc_ckpt':
                osp.join(BASE_MODEL_DIR, 'basemodel_16k', 'hifigan', 'ckpt'),
                'voc_config':
                osp.join(BASE_MODEL_DIR, 'basemodel_16k', 'hifigan',
                         'config.yaml'),
                'audio_config':
                osp.join(model_dir, 'data', 'audio_config.yaml'),
                'se_file':
                osp.join(model_dir, 'data', 'se', 'se.npy')
            }
            kwargs = {'custom_ckpt': custom_infer_abs}
            if not os.path.exists(os.path.join(model_dir, "configuration.json")):
                os.symlink(os.path.join(BASE_MODEL_DIR, "configuration.json"), os.path.join(model_dir, "configuration.json"))
            if not os.path.exists(os.path.join(model_dir, "resource.zip")):
                os.symlink(os.path.join(BASE_MODEL_DIR, "resource.zip"), os.path.join(model_dir, "resource.zip"))
            if not os.path.exists(os.path.join(model_dir, "resource")):
                os.symlink(os.path.join(BASE_MODEL_DIR, "resource"), os.path.join(model_dir, "resource"))
            model_id = SambertHifigan(model_dir, **kwargs)
            NAIVE_LOG('Model constructed for inference: {}'.format(model_id), user_id)
...
def training_task(name, cur_data_path, cur_model_path, session):
    user_id = session.get('modelscope_uuid', 'guest')
    try:
        NAIVE_LOG('Training task started', user_id)
        autolabel_work_dir = osp.join(cur_data_path, 'autolabel')
        os.makedirs(autolabel_work_dir, exist_ok=True)


        NAIVE_LOG('Before AutoLabeling', user_id)
        auto_labeling = AutoLabeling(os.path.abspath(cur_data_path),
                                     AUTO_LABEL_RESOURCE_DIR,
                                     False,
                                     os.path.abspath(autolabel_work_dir),
                                     'female',
                                     1,
                                     False,
                                     'all',
        ...
        NAIVE_LOG('After AutoLabeling', user_id)
        if ret_code != 0:
            NAIVE_LOG("AutoLabeling failed: {}".format(report), user_id)
            increment_training_done_count()
            return False, "数据处理失败，请在相对安静的环境下重新录音", session
        #  progress(0.6, desc='Build trainer')

        # train
        pretrained_model_id = 'damo/speech_personal_sambert-hifigan_nsf_tts_zh-cn_pretrain_16k'
        #   dataset_id: <workspace>/<uuid>/data/202303142248/autolabel
        dataset_id = autolabel_work_dir
        ...
        #   pretrain_work_dir: <workspace>/<uuid>/models/202303142248/pretrain
        ...
        NAIVE_LOG("Before build modelscope trainer", user_id)
        trainer = build_trainer(Trainers.speech_kantts_trainer,
                                default_args=kwargs)
        NAIVE_LOG("After build modelscope trainer", user_id)

        NAIVE_LOG("Before modelscope trainer training", user_id)
        trainer.train()
        NAIVE_LOG("After modelscope trainer training", user_id)
...
def launch_training_task(uuid_txt, session, *audio_lst):
    ...
    before_queue_size = training_threadpool._work_queue.qsize()
    before_done_count = training_done_count

    NAIVE_LOG("Launching training task...", user_id)
...
with gr.Blocks(css="#warning {color: red} .feedback {font-size: 24px}") as demo:
    ...
    with gr.Tabs():
        with gr.TabItem("\N{rocket}模型定制") as tab_train:
            ...
            \N{face savouring delicious food}  移动端现已支持啦
